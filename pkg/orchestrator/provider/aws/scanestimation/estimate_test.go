...
func Test_getScanSize(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := getScanSize(tt.args.stats, tt.args.asset, tt.args.input)
...
func Test_getScanDuration(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// 1 * 2.5 (static time * scan size) for  Misconfigurations
			// 360 * 2.5 for Malware from static table
			// 1 * 2.5 (static time * scan size) for  Vulnerabilities
			wantDuration: 1314,
			wantErr:      false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotDuration, err := getScanDuration(tt.args.stats, tt.args.familiesConfig, tt.args.scanSizeMB, tt.args.input)
			if (err != nil) != tt.wantErr {
