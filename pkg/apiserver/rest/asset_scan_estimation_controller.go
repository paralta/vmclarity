...
func (s *ServerImpl) GetAssetScanEstimationsAssetScanEstimationID(ctx echo.Context, assetScanEstimationID models.AssetScanEstimationID, params models.GetAssetScanEstimationsAssetScanEstimationIDParams) error {
	...
	if err != nil {
		...
		return sendError(ctx, http.StatusInternalServerError, fmt.Sprintf("failed to get asset scan Estimation from db. assetScanID=%v: %v", assetScanEstimationID, err))
	}
...
func (s *ServerImpl) PatchAssetScanEstimationsAssetScanEstimationID(ctx echo.Context, assetScanEstimationID models.AssetScanEstimationID, params models.PatchAssetScanEstimationsAssetScanEstimationIDParams) error {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return sendError(ctx, http.StatusNotFound, fmt.Sprintf("asset scan Estimation was not found. assetScanID=%v: %v", assetScanEstimationID, err))
		}
	...
	if err != nil {
		...
		default:
			return sendError(ctx, http.StatusInternalServerError, fmt.Sprintf("failed to update asset scan Estimation in db. assetScanID=%v: %v", assetScanEstimationID, err))
		}
	}
